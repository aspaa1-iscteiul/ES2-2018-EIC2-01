...
public class ProblemIdPage extends SuperPage{
	...
	public void createMainPanel() {
		...
		problemName.setBorder(BorderFactory.createCompoundBorder(border,BorderFactory.createEmptyBorder(0, 10, 0, 10)));
		problemName.addKeyListener(new KeyListener() {
			...
			public void keyReleased(KeyEvent e) {
				try {
					// validates the email
					if(Pattern.matches("[a-zA-Z0-9]+", problemName.getText()) && Character.isUpperCase(problemName.getText().charAt(0))) {	
						validName = true;
					} else {
						validName = false;
					}
				} finally {
					nextButton.setEnabled(validName);
				}
			}
		...
		JLabel infoLabel = new JLabel("<html>The name you choose must start with a capital letter and can only <br> contain numbers and letters (from A to Z).<br></html>");
